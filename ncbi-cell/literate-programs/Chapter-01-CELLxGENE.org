...
queried, and analyzed using the
[[https://chanzuckerberg.github.io/cellxgene-census/][CELLxGENE
Discover Census]].  Using cell-based slicing and querying one can:
...
Then call it to obtain the human lung cell metadata and datasets, and
print the result:
...
[[file:Chapter-02-E_Utilities.org][Chapter-02-E_Utilities.org]]). So, we examine the ~collection_name~ and
~dataset_title~ columns:
...
Next we call the function for an example citation:
...
~wget~, since some journals respond to one, but not the other. The
...
Next, in [[file:Chapter-02-E_Utilities.org][Chapter-02-E_Utilities.org]], we write a function to search
PubMed for the title.
